...
export class StripePaymentProvider implements PaymentProvider {
  ...
  async createConnectedAccount(params: {
    ...
    try {
      const account = await this.stripe.accounts.create(
        ...
        { idempotencyKey: `account_${params.ownerId}` },
      );

      return { connectedAccountId: account.id };
    } catch (error) {
      throw this.classifyStripeError(error, 'createConnectedAccount', true);
    }
  }
  ...
  async createOnboardingLink(params: {
    ...
    try {
      ...
      throw this.classifyStripeError(error, 'createOnboardingLink', false);
    }
  }
  ...
  async createCheckoutSession(params: {
    ...
    try {
      const session = await this.stripe.checkout.sessions.create(
        ...
        { idempotencyKey: `checkout_${params.rentalId}` },
      );
      ...
      throw this.classifyStripeError(error, 'createCheckoutSession', true);
    }
  }
  ...
  async capturePayment(paymentIntentId: string): Promise<{ captured: boolean }> {
    try {
      ...
      throw this.classifyStripeError(error, 'capturePayment', true);
    }
  }
  ...
  async refundPayment(paymentIntentId: string): Promise<{ refunded: boolean }> {
    try {
      ...
      throw this.classifyStripeError(error, 'refundPayment', true);
    }
  }
  ...
  async transferToConnectedAccount(params: {
    ...
    try {
      ...
      throw this.classifyStripeError(error, 'transferToConnectedAccount', true);
    }
  }
  ...
  /**
   * Classify Stripe SDK errors into typed ProviderErrors.
   *
   * Maps each Stripe error subclass to a specific ProviderErrorCode.
   * The `isStateChanging` flag determines whether a connection error
   * is classified as ambiguous (state-changing) or simply unavailable.
   */
  private classifyStripeError(
    error: unknown,
    operation: string,
    ...
    return new ProviderError({
      message: `${operation}: ${error instanceof Error ? error.message : 'unknown'}`,
